...
const tmContent = {
    ...
    heroHeading: "Transform Your Logistics Career with the <gradient>#1 SAP TM Online Training Program</gradient>",
    ...
    badges: [],
    // heroStats is often not stored in DB if not in schema, but we can try putting it in 'features' or ignored if not in DB schema.
    // Schema usually has: features, sections, curriculum, faqs, badges, hero_image, etc.
    heroStats: [
...
async function restoreTmContent() {
    console.log(`Restoring full TM content for ID: ${tmContent.id}`);

    // Update main fields
    const { error } = await supabase
        .from('courses')
        .update({
            title: tmContent.title,
            hero_heading: tmContent.heroHeading,
            hero_subheading: tmContent.heroSubheading,
            description: tmContent.description,
            price: tmContent.price,
            duration: tmContent.duration,
            syllabus_url: tmContent.syllabusUrl,
            meta_title: tmContent.metaTitle,
            meta_description: tmContent.metaDescription,
            // features is string[] in interface but DB might use different logic. 
            // In courses.json, features is string[].
            // But tmContent.sections has 'detailed_features'. 
            // We'll update what we have.
            sections: tmContent.sections,
            // Assuming curriculum, faqs, badges are handled if columns exist
            badges: tmContent.badges,
            // heroStats
            // We don't have direct column for heroStats usually, but let's try.
        })
        .eq('id', tmContent.id);
    ...
    if (error) {
        ...
        console.log('Successfully restored complete TM course content!');
    }
}
