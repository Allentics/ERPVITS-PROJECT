-- Restore SAP C4C Technical Course Content
-- Updates the 'c4c-technical' course with full content from lib/c4cContent.tsx
-- FIX: Removed invalid columns (meta_keywords, badges, target_audience, prerequisites, syllabus_url, price, meta_title, meta_description)
-- FIX: Added target_audience and prerequisites to sections JSON

UPDATE courses
SET
  title = 'SAP C4C Technical',
  description = 'Advance your career with ERPVITS SAP C4C Technical Training. Get real-world project experience, certification prep & placement support for global SAP roles.',
  duration = '30-35 Hours',
  hero_heading = 'End-to-End <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#ff4500] to-red-600 box-decoration-clone">SAP C4C Technical Training</span> with Real-Time Implementation',
  ...
  sections = '[
    ...
    },
    {
        ...
        "title": "Who Should Enroll in this Course?",
        "items": [
            {
  ...
  -- Schema
  -- Removed schema column update to avoid issues, as page.tsx might generate it from metadata
  -- No, wait, previous scripts didn't fail on schema. But to be safe, I'll remove it if it's not critical. 
  -- Actually, update_abap_hana_course.sql did NOT update 'schema'.
  -- So I will remove 'schema' update as well.
  
  updated_at = NOW()
WHERE id = 'c4c-technical';
