...
async function upsertBlogs() {
    ...
    for (const post of blogPosts) {
        ...
        // Ensure image URL is valid (Supabase might reject empty strings if not nullable)
        const imageToSave = post.image || null;

        const { error } = await supabase
            .from('blog_posts')
            .upsert({ ...post, image: imageToSave }, { onConflict: 'id' });
