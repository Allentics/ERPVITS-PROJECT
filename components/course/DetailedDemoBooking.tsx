...
export default function DetailedDemoBooking({ title, subtitle, courseName = "this course", benefits, features, syllabusUrl }: { title?: string | React.ReactNode, subtitle?: string | React.ReactNode, courseName?: string, benefits?: any[], features?: any[], syllabusUrl?: string }) {
    ...
    return (
        <section id="detailed-demo-booking" className="py-8 bg-orange-50 border-y border-orange-100 relative overflow-hidden">
            <div className="max-w-6xl mx-auto px-4 relative z-10">
                <div className="text-center mb-6">
                    <span className="bg-orange-500 text-white px-3 py-0.5 rounded-full text-[10px] font-bold tracking-widest uppercase mb-2 inline-block">
                        Get Started
                    </span>
                    <h2 className="text-xl font-bold text-slate-900 mb-2">
                        {title || <>Ready to Become an <span className="text-orange-500">{courseName} Consultant?</span></>}
                    </h2>
                    <p className="text-slate-600 max-w-xl mx-auto text-[11px]">
                ...
                <div className="grid lg:grid-cols-2 gap-10 items-start">
                    <div className="space-y-4">
                        <div>
                            <h3 className="text-sm font-bold text-slate-800 mb-3">What You'll Get With Your Free Demo</h3>
                            <div className="space-y-2">
                                {displayBenefits.map((itemValue: string | { title: string; desc?: string; description?: string; icon?: React.ElementType }, i: number) => {
                                    ...
                                    return (
                                        <div key={i} className="bg-white border border-orange-100 p-2.5 rounded-lg flex items-start gap-3 shadow-sm">
                                            <div className="w-6 h-6 rounded bg-orange-500 flex items-center justify-center text-white shrink-0">
                                                <Icon className="w-3.5 h-3.5" />
                                            </div>
                                            <div>
                                                <h4 className="font-bold text-slate-800 mb-0 text-[11px]">{itemTitle}</h4>
                                                {itemDesc && <p className="text-[9px] text-slate-500">{itemDesc}</p>}
                                            </div>
                                        </div>
                                    )
                                })}
                    ...
                    {/* Right Column: Form */}
                    <div className="bg-white rounded-xl p-4 lg:p-5 shadow-lg border border-orange-100 relative">
                        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-orange-400 to-orange-500 rounded-t-xl"></div>
                        <h3 className="text-xl font-bold text-slate-900 mb-6 text-center">Book Your <span className="text-orange-500">Free Demo</span></h3>
