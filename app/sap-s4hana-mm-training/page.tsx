...
export async function generateMetadata(): Promise<Metadata> {
    ...
    const local = courses.find(c => c.id === 'mm' || c.id === 'sap-mm');

    return {
        title: course?.meta_title || local?.metaTitle || `${course?.title || local?.title} Online Training | ERPVITS`,
        description: course?.meta_description || local?.metaDescription || `Master ${course?.title || local?.title} with ERPVITS - Live online training.`,
        alternates: {
...
export default function Page() {
    return <CoursePage params={Promise.resolve({ slug: 'sap-mm' })} />;
}
